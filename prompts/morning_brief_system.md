...
8. Output plain Markdown, no code fences, no horizontal rules (---), no preamble, no closing pleasantries. Write in {language}, including the section headings (translate them; do not print bilingual headings).
...
**Data caveats** — one bullet per item in `systemic_flags` and `data_quality`. State clearly that these are reporting artefacts and, where the note says so, that conversion-based KPIs for the recent days are provisional.
